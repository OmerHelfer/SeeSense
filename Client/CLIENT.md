...
| **AdminStatus** | The performance dashboard. Auto-refreshes every 3 s. Eleven range presets (חי / מההתחלה / 30 דק׳ / שעה / יום / שבוע / חודש / 3 ח׳ / 6 ח׳ / שנה / מותאם) plus a custom `datetime-local` picker converted to epoch seconds. Stat cards (uptime or measured span, FPS, frames, throughput), an FPS comparison table (client actual vs server actual/capacity/overall), a latency comparison (**שרת בלבד** / **לקוח בלבד** / **RTT (רשת + שרת)** / **E2E Latency** + an estimated network figure). Note these are four different spans, not four addends: **RTT** is the wire round trip (`send` → result), the **network** figure is not measured but derived as `RTT − server`, **לקוח בלבד** is the pre-send capture+encode cost that sits *outside* the RTT, and **E2E Latency** is the only number that spans the whole user-facing pipeline (capture → feedback) — it contains the RTT rather than sitting beside it. Then per-stage breakdowns for both server and client — the server's last two rows, **כתיבה ל-DB (צרור שלם)** and **כתיבה ל-DB (לרשומה)**, are the same batch-write cost undivided and divided by the record count, and *neither* is part of the frame total since the write happens on a background thread after the reply is sent. Finally a **hand-drawn canvas RTT chart** with grid, gradient fill, threshold lines at 100/150/200 ms, and a span label computed from real timestamps. The reset-everything button is gated to level 2. |
